...
function getItemParams(files) {
  ...
  const mdData = fs.readFileSync(
    path.join(process.cwd(), `${dir}/${base}`),
    'utf-8'
  );
...
const isMd = (file) => /.(md|MD)$/.test(file);
  ...
  list.forEach((file) => {
    ...
    if (isDir(filePath)) {
      ...
      fs.readdirSync(filePath).forEach((item) => {
        ...
        if (isDir(itemPath)) {
          addDirItem(fs.readdirSync(itemPath), `${dir}/${file}/${item}`);
        }
      });
...
const createSidebar = (dirPath) => {
  ...
  const filterFiles = files.filter(
    (file) => ![...defaultIngoreList, ...ingoreDirList].includes(file)
  );
...
async function generateSidebar(config) {
  const { dir } = config;
  return sortSidebar(createSidebar(dir));
}

export default function autoSidebar(optConfig: ConfigInterface) {
  return {
    name: 'autoSidebar',
    async config(config) {
      options = {
      ...
      console.log(
        chalk.blue('[sidebar]'),
        chalk.green('insert sidebar data successfully')
      );
      return config;
    },
    configureServer(serve) {
      ...
      mdWatcher.on('all', async (event) => {
        if (event !== 'change') {
          ...
          console.log(
            chalk.blue('[sidebar]'),
            chalk.green('update sidebar data successfully')
          );
        }
      });
    }
