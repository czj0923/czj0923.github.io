...
export function useElementSize(
  ...
  const isSVG = computed(() =>
    unrefElement(target)?.namespaceURI?.includes('svg')
  );
  ...
  const { stop: stop1 } = useResizeObserver(
    ...
    ([entry]) => {
      ...
      //[!code ++]
      // 对于 SVG 元素，ResizeObserver 报告的尺寸可能不总是符合预期，所以需要特殊处理
      if (window && isSVG.value) {
        ...
        if (boxSize) {
          // [!code ++]
          // ResizeObserver的borderBoxSize 目前是只有一个元素的数组，未来可能会有多个元素，所以这么处理
